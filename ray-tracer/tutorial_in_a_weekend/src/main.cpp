...
int main(int argc, char *argv[])
{
  ...
  try {
    ...
    std::cerr << "No camera file found (" << e.what() << ")" << std::endl;
    return -2;
  }
  // Camera
      ...
      */
      ...
      for (int i = 0; i < render_params.width; ++i) {
        ...
        for(int s = 0; s < render_params.min_samples_per_pixel / 2; s++) {
          ...
          auto tmpc1 = ray_color(cam.get_ray(u, v), world.background, world.objects, world.lights, render_params.max_depth);
          auto tmpc2 = ray_color(cam.get_ray(u, v), world.background, world.objects, world.lights, render_params.max_depth);
	  ...
	  if(
        ...
        // Adaptive loop
        for(int s = render_params.min_samples_per_pixel / 2; s < render_params.max_samples_per_pixel / 2; s++) {
          auto u = (i + random_double()) / ((double)render_params.width - 1);
          auto v = (j + random_double()) / ((double)render_params.height - 1);
